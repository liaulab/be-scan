...
def find_bounds(df_scaled, gene, gene_map, domain): 
    if len(gene_map) == 0: # FOR ONE GENE
        # TRY START BOUND
        start = domain['start']
...
def plot_PWES_heatmap(df_scaled, gene_map, out_prefix, out_dir, 
                      bounds={}, show_plots=True, 
                      mask_on=True, sns_context='talk', cmap='RdBu_r', v_min=-1, v_max=1):
    ...
    if mask_on:
        mask[np.tril_indices_from(mask, k=-1)] = True # MASK FOR LOWER TRIANGLE #
    
    # GENERATE HEATMAP #
    fig, ax = plt.subplots(figsize=(15, 12))
    sns.heatmap(data=df_scaled, ax=ax, 
    ...
    for gene, bound in bounds.items():
        for domain in bound: 
            start, end = find_bounds(df_scaled, gene, gene_map, domain)

            if len(start) < 1 or len(end) < 1 or start[0] >= end[0]: 
                domain_name = domain['name']
                print(f'Error drawing domain {domain_name} in gene {gene}')
                continue
            
            # ADD IN DOMAIN RANGES #
            ...
            mid_x, mid_y = (start[0] + end[0]) / 2, (start[0] + end[0]) / 2
            x_lim, y_lim = ax.get_xlim(), ax.get_ylim()
            ax.text(x_lim[1]+5, mid_y, domain['name'], fontsize=10, 
                    ha='left', va='center', color='black', rotation=0)
            ax.text(mid_x, y_lim[1]-5, domain['name'], fontsize=10, 
...
def plot_clust_histogram(df_clus, out_prefix, out_dir, color_map, show_plots=True):
    """
    ...
    plt.ylabel("Count")
    plt.title("Number of Guides in Clusters")
...
def plot_clust_boxplots(df_clus, x_col, out_prefix, out_dir, color_map, show_plots=True): 
    """
    Given df_clus, generated from cluster_pws, plot boxplot of the values for each cluster of guides.
    """
    try: 
        df_clus['cl_new']
    except KeyError:
...
def plot_clustermap(df_scaled, link, df_clusters, 
                    out_prefix, out_dir, color_list, color_clusters=True, show_plots=True, 
...
# def plot_PWES_heatmap_clust(df_pwes_sorted, aas_dict, out_prefix, out_dir, 
#                             mask_on=True, bounds=[], 
...
#         # LOOK AT DOMAINS
#         for bound in bounds:
#             start, end = find_bounds(df_pwes_sorted, gene, gene_map, bounds)
#             # TRY START BOUND
#             start = bound['start']
...
#                 print(f'Error drawing domain {domain_name}')
#                 continue
...
### HELPER FUNCTIONS: SCATTERPLOTS--------------------------------------------------------------------------

def plot_scatter(df_clus, x_col, scores_col, 
                 out_prefix, out_dir, color_map, show_plots=True):
    """
    Generates scatter plots colored by each cluster in `cl_new`
    """
    ...
    try: 
        ...
        raise Exception('df_clus does not contain a "cl_new" column')
    width = df_clus[x_col].max()/50

    fig, ax = plt.subplots(figsize=(width, 6))
    scatter = sns.scatterplot(data=df_clus, x=x_col, y=scores_col, 
                              hue='cl_new', palette=color_map, ax=ax)
...
def plot_subscatter(df_clus, x_col, scores_col, 
                    out_prefix, out_dir, color_map, show_plots=True, cols=3):
    """
    Generates scatter plots colored by each cluster in `cl_new` 
    for each cluster while coloring other points gray.
    """
    ...
    plt.tight_layout()
    plt.draw()
    fig.savefig(f'{out_dir}/{out_prefix}_subscatterplot.pdf', format='pdf')
    fig.savefig(f'{out_dir}/{out_prefix}_subscatterplot.png', format='png')
    if show_plots: 
        plt.show()
    plt.close(fig)
    
def plot_scatter_complex(df_clus, x_col, scores_col, 
                         out_prefix, out_dir, color_map, gene_map, show_plots=True):
    try: 
        df_clus['cl_new']
    except KeyError:
        raise Exception('df_clus does not contain a "cl_new" column')
    ...
    unique_chains = df_clus['chain'].unique()
    gene_map_unique_chains = list(gene_map.values())
    common_chains = set(unique_chains) and set(gene_map_unique_chains)
    width = df_clus[x_col].max()/50

    fig, ax = plt.subplots(len(common_chains), 1, 
                           figsize=(width, 6 * len(common_chains)))
    ...
    for i, chain in enumerate(common_chains): 
        ...
        ax[i].set_title(f"Chain {chain} Colored by Clusters")
    ...
    # REMOVE DUPES FOR LEGEND ITEMS #
    all_handles, all_labels = sum(handles_list, []), sum(labels_list, [])
    seen = set()
...
def plot_subscatter_complex(df_clus, x_col, scores_col, 
                            out_prefix, out_dir, color_map, gene_map, show_plots=True):
    """
    ...
    fig, axes = plt.subplots(num_clusters, len(common_chains), 
                             figsize=(len(common_chains) * width, num_clusters * 3))  # Create subplots
    ...
    plt.tight_layout()
    plt.draw()
    fig.savefig(f'{out_dir}/{out_prefix}_subscatterplots.pdf', format='pdf')
    fig.savefig(f'{out_dir}/{out_prefix}_subscatterplots.png', format='png')
    if show_plots: 
        plt.show()
    plt.close(fig)

### HELPER FUNCTIONS: CIRCOS PLOT--------------------------------------------------------------------------
