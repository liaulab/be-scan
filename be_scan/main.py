...
### plot ###
    ...
    parser_plot_scatterplot.add_argument('-df', '--df_filepath', type=str, help='filepath to .csv data generated from count_reads', required=True)
    parser_plot_scatterplot.add_argument('-x', '--x_column', type=str, help='column of .csv, typically amino acid position', required=True)
    parser_plot_scatterplot.add_argument('-y', '--y_column', type=str, help='column of .csv, typically the normalized log_fc score', required=True)
    parser_plot_scatterplot.add_argument('-hue', '--hue_column', type=str, help='column of .csv which correspond to coloring of points', required=True)
    parser_plot_scatterplot.add_argument('-c','--comparisons', nargs='+', type=str, help='list of comparisons that correspond to columns of data', required=True)
    parser_plot_scatterplot.add_argument('-ncol', '--neg_ctrl_col', type=str, help='column of .csv which correspond to normalization control', required=True)
    parser_plot_scatterplot.add_argument('-ncat', '--neg_ctrl_category', type=str, help='categorical variable of neg_ctrl_coll', required=True)
    # optional variables
    ...
    # required args
    parser_plot_boxes.add_argument('-df', '--df_filepath', type=str, help='filepath to .csv data generated from count_reads', required=True)
    parser_plot_boxes.add_argument('-p', '--plot_column', type=str, help='column of .csv, typically domain or mutation type', required=True)
    parser_plot_boxes.add_argument('-pc', '--plot_conditions', nargs='+', type=str, help='category names of plot_column', required=True)
    parser_plot_boxes.add_argument('-y', '--y_column', type=str, help='column of .csv, typically the normalized log_fc score', required=True)
    parser_plot_boxes.add_argument('-c', '--comparisons', nargs='+', type=str, help='list of comparisons that correspond to columns of data', required=True)
    parser_plot_boxes.add_argument('-ncol', '--neg_ctrl_col', type=str, help='column of .csv which correspond to normalization control', required=True)
    parser_plot_boxes.add_argument('-ncat', '--neg_ctrl_category', type=str, help='categorical variable of neg_ctrl_col', required=True)
    # optional variables
    ...
    # required args
    parser_plot_corr_heatmap.add_argument('-df', '--df_filepath', type=str, help='filepath to .csv data generated from count_reads', required=True)
    parser_plot_corr_heatmap.add_argument('-c','--comparisons', nargs='+', type=str, help='list of comparisons that correspond to columns of data', required=True)
    # optional args
    parser_plot_corr_heatmap.add_argument('--corr_type', type=str, default='spearman', help='type of correlation calculation')
    ...
    parser_plot_corr_scatterplot.add_argument('-df', '--df_filepath', type=str, help='filepath to .csv data generated from count_reads', required=True)
    parser_plot_corr_scatterplot.add_argument('-c1', '--condition1', type=str, help='comparison condition 1, name of a column in .csv data', required=True)
    parser_plot_corr_scatterplot.add_argument('-c2', '--condition2', type=str, help='comparison condition 2, name of a column in .csv data', required=True)
    parser_plot_corr_scatterplot.add_argument('-hue', '--hue_column', type=str, help='the categorial dimension of the data, name of .csv data column', required=True)
    # optional args
    parser_plot_corr_scatterplot.add_argument('--hue_order', nargs='+', type=str, default=list_muttypes, help='a list of categorial variables in hue_column')
    parser_plot_corr_scatterplot.add_argument('--palette', nargs='+', type=str, default=color_list, help='a list of colors which correspond to hue_order')
