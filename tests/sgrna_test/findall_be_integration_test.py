...
def test_findall_BE_basic_integration_pos(): 
    out = subprocess.run("python3 -m bigscam findall_be -g tests/test_data/230408_AR_Input.fasta -e1 C -e2 T -c Sp -p tests/test_data/P10275.fasta", shell=True, capture_output=True)
    assert out.returncode == 0
    assert filecmp.cmp("_Sp_CtoT_library.csv", "tests/ref_data/AR_CBESp_library.csv")
    os.system("rm _Sp_CtoT_library.csv")
    out = subprocess.run("python3 -m bigscam findall_be -g tests/test_data/230408_AR_Input.fasta -e1 A -e2 G -c Sp -p tests/test_data/P10275.fasta", shell=True, capture_output=True)
    assert out.returncode == 0
    ...
    out = subprocess.run("python3 -m bigscam findall_be -g tests/test_data/230408_AR_Input.fasta -e1 A -e2 G -c SpG -p tests/test_data/P10275.fasta", shell=True, capture_output=True)
    assert out.returncode == 0
    assert filecmp.cmp("_SpG_ABE_library.csv", "tests/ref_data/AR_ABESpG_library.csv")
    os.system("rm _SpG_AtoG_library.csv")

    out = subprocess.run("python3 -m bigscam findall_be -g tests/test_data/230408_AR_Input.fasta -e1 C -e2 T -c SpRY -p tests/test_data/P10275.fasta", shell=True, capture_output=True)
    assert out.returncode == 0
    assert filecmp.cmp("_SpRY_CBE_library.csv", "tests/ref_data/AR_CBESpRY_library.csv")
    ...
    assert out.returncode == 0
    assert filecmp.cmp("_SpRY_ABE_library.csv", "tests/ref_data/AR_ABESpRY_library.csv")
...
def test_findall_BE_opt_guide_length_integration_pos():
    ...
    assert out.returncode == 0
    # assert filecmp.cmp("_SpG_CBE_library.csv", "tests/ref_data/AR_CBESpG_len26_library.csv")
    os.system("rm _SpG_CtoT_library.csv")
