...
def test_generate_BE_guides_basic_optional_pos(edit_from, edit_to, cas_type, 
    ...
    df = generate_BE_guides(gene_filepath=AR_filepath,
                            gene_name="AR", edit_from=edit_from, edit_to=edit_to, cas_type=cas_type, 
                            PAM=PAM, 
                            window=window, 
...
def test_generate_BE_guides_window_neg(): 
    ...
    with pytest.raises(AssertionError): 
        df = generate_BE_guides(gene_filepath=AR_filepath,
                                gene_name="AR",
                                edit_from="C", edit_to="T", cas_type='Sp', 
...
def test_generate_BE_guides_pam_neg(): 
    ...
    with pytest.raises(AssertionError): 
        df = generate_BE_guides(gene_filepath=AR_filepath,
                                gene_name="AR",
                                edit_from="C", edit_to="T", cas_type='Sp', window=(4,8),
...
def test_generate_BE_guides_excludeintrons_pos(): 
    ...
    df1 = generate_BE_guides(gene_filepath=AR_filepath,
                             gene_name="AR",
                             edit_from="C", edit_to="T", cas_type='Sp', window=(4,8),
                             exclude_introns=True, save_df=False
                    )
    df2 = generate_BE_guides(gene_filepath=AR_filepath,
                             gene_name="AR",
                             edit_from="C", edit_to="T", cas_type='Sp', window=(4,8),
